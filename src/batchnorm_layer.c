...
layer16 make_batchnorm_layer16(int batch, int w, int h, int c)
{
    fprintf(stderr, "Batch Normalization Layer: %d x %d x %d image\n", w,h,c);
    layer16 l = {0};
    l.type = BATCHNORM;
    ...
    l.w = l.out_w = w;
    l.c = l.out_c = c;
    l.output = calloc(h * w * c * batch, sizeof(FLT));
    l.delta  = calloc(h * w * c * batch, sizeof(FLT));
    l.inputs = w*h*c;
    l.outputs = l.inputs;

    l.scales = calloc(c, sizeof(FLT));
    l.biases = calloc(c, sizeof(FLT));

    int i;
    for(i = 0; i < c; ++i){
        l.scales[i] = 1;
    }
    l.rolling_mean = calloc(c, sizeof(FLT));
    l.rolling_variance = calloc(c, sizeof(FLT));

    l.forward = forward_batchnorm_layer16;
    //l.backward = backward_batchnorm_layer;
    return l;
}
...
void saveresbnf( char * filename,float *data, int len,int idx)
{
...
void saveresbn( char * filename,FLT *data, int len,int idx)
{
	FILE *fp = fopen(filename ,"w+");
	int num;
	for(num=0;num<len;num++){
...
int g_bn=0;
void forward_batchnorm_layer16(layer16 l, network16 net)
{
    //printf("forward_batchnorm_layer16 :flag_vec = %d \n",net.flag_vec);
    if(l.type == BATCHNORM) copy_cpu16(l.outputs*l.batch, net.input, 1, l.output, 1);
    copy_cpu16(l.outputs*l.batch, l.output, 1, l.x, 1);

    normalize_cpu16(l.output, l.rolling_mean, l.rolling_variance, l.batch, l.out_c, l.out_h*l.out_w);

    scale_bias16(l.output, l.scales, l.batch, l.out_c, l.out_h*l.out_w);
    add_bias16(l.output, l.biases, l.batch, l.out_c, l.out_h*l.out_w);
}
...
{
    if(l.type == BATCHNORM) { printf("BATCHNORM\n");copy_cpu(l.outputs*l.batch, net.input, 1, l.output, 1);}
